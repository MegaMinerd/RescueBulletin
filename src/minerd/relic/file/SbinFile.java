...
public class SbinFile extends BufferedDataHandler {
	...
	public SbinFile(ByteBuffer bufferIn, String nameIn, int offsetIn) {
		//TODO: Currently using getAll(). Getting a trimmed segment would be better.
		super(bufferIn);
		this.name = nameIn;
		...
		try{
			...
			//addSegment(names[count-1], pointers[count-1].getOffset(), length());
	...
	private void addSegment(String filename, int start, int end) throws IOException {
		try{
			...
			//TODO: Find a better end detection method
		}
	}
